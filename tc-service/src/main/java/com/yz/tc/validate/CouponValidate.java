...
public class CouponValidate {
    ...
    public static boolean validateForCreate(Coupon coupon, TCResult<?> message, boolean isUpdateFlag) {
        ...
        if (coupon.getPerOrderLimit().intValue() < 1) {
            message.setErrorCode(TCResultCode.CAN_NOT_BE_OPERATE, "每笔订单优惠券使用数量", "小于1");
            return false;
        }
    ...
    private static boolean couponTimeCheck(TCResult<?> message, Coupon coupon, Date issueStartTime, Date issueEndTime, String target) {
        ...
        if (issueEndTime == null) {
            message.setErrorCode(TCResultCode.FIELD_NOT_ALLOWED_EMPTY, target + "发券结束时间");
            return false;
        }
        // 优惠券发券开始时间大于等于发券结束时间 报错
        if (issueStartTime.getTime() >= issueEndTime.getTime()) {
            message.setErrorCode(TCResultCode.TIME_VALUE_MUST_LESS_THAN, target + "发券开始时间", "发券结束时间");
            return false;
        }
        // 优惠券预热开始时间大于等于发券开始时间 报错
        if (coupon.getPendingTime() != null
                && coupon.getPendingTime().getTime() >= issueStartTime.getTime()) {
            message.setErrorCode(TCResultCode.TIME_VALUE_MUST_LESS_THAN, "预热时间", target + "发券开始时间");
            return false;
        }
        ...
        if (issueEndTime.getTime() > coupon.getEndTime().getTime()) {
            message.setErrorCode(TCResultCode.TIME_VALUE_MUST_LESS_THAN, target + msg + "结束时间", "优惠券失效时间");
            return false;
        }
        // 优惠券发券开始日期大于生效开始时间 报错
        if (issueStartTime.getTime() > coupon.getStartTime().getTime()) {
            message.setErrorCode(TCResultCode.TIME_VALUE_MUST_LESS_THAN, target + msg + "开始时间", "优惠券生效时间");
            return false;
        }
        return true;
    }
    ...
    public static boolean validateForUpdate(Coupon coupon, TCResult<?> message) {
        ...
        if (coupon.getId().intValue() <= 0) {
            message.setErrorCode(TCResultCode.COMMON_PARAM_INVALID, Coupon.ID);
        }
